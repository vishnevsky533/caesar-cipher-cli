...
test("It should check for required options", () => {
  ...
  expect(parse({ a: "code", i: "c:/input.txt", o: "./output.txt" })).toBeNull();
  expect(parse({ s: "5", i: "c:/input.txt", o: "./output.txt" })).toBeNull();
  expect(
  ...
  expect(parse({ i: "c:/input.txt", o: "./output.txt" })).toBeNull();
  expect(
